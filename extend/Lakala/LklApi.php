...
class LklApi
{
    ...
    public static function lklPreorder($param)
    {
        $sepParam = [
            ...
            'term_no' => $param['term_nos'],
            'out_trade_no' => $param['order_no'],
            'account_type' => $param['account_type']??'WECHAT', //钱包类型 微信：WECHAT 支付宝：ALIPAY 银联：UQRCODEPAY 翼支付: BESTPAY 苏宁易付宝: SUNING 拉卡拉支付账户：LKLACC 网联小钱包：NUCSPAY 京东钱包：JD
        ...
        // 根据支付类型设置不同的业务字段
        if ($sepParam['account_type'] == 'ALIPAY') {
            // 支付宝主扫支付业务字段
            $sepParam['acc_busi_fields'] = [
                'timeout_express' => '15',
                'user_id' => $param['openid'], // 支付宝用户ID
                'goods_detail' => [
                    [
                ...
                'auth_code' => $param['auth_code'] ?? '', // 支付宝付款码
                'store_id' => self::$config['app_id'], // 门店编号
            ];
        } else {
            // 微信支付业务字段
            $sepParam['acc_busi_fields'] = [
    ...
    public static function lklRefund($param)
    {
        $sepParam = [
            ...
            'origin_log_no' => $param['origin_log_no'],
    ...
    public static function lklSeparate($param)
    {
        $sepParam = [
            ...
            'cal_type' => $param['cal_type'], //分账计算类型 0- 按照指定金额，1- 按照指定比例。默认 0
