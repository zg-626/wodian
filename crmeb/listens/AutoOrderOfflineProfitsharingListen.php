...
class AutoOrderOfflineProfitsharingListen extends TimerService implements ListenerInterface
{
    ...
    public function handle($event): void
    {
        $this->tick(30000, function () {
            ...
            $models =$storeOrderProfitsharingRepository ->getAutoOfflineProfitsharing();
            foreach ($models as $model) {
                Log::info('线下自动分账' . var_export($model->order_id, 1));
                $storeOrderProfitsharingRepository->partnerProfitsharing($model);
                //Queue::push(OrderOfflineProfitsharingJob::class, $id);
            }
        });
    }
}
