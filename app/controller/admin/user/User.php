...
//        if (!$data['birthday']) unset($data['birthday']);
     ...
     * @desc 修改上级
     **/
    public function proxyForm($id)
    {
        if (!$this->repository->exists((int)$id))
            return app('json')->fail('数据不存在');
        return app('json')->success(formToData($this->repository->changeProxyForm($id)));
    }
    ...
    public function excel()
    {
        ...
        [$page, $limit] = $this->getPage();
        $data = app()->make(ExcelService::class)->user($where, $page, $limit);
        return app('json')->success($data);
    }
    ...
    public function batchSpread()
    {
        ...
        $this->repository->updates($uids, ['is_promoter' => $status]);
        return app('json')->success('修改成功');
    }
}
