    ...
    public function form(?int $id,array $formData = [])
    {
        ...
        $form->setRule([
            ...
            Elm::textarea('guarantee_info', '服务内容描述：')->autosize([
                'minRows'=>1000,
            ])->placeholder('请输入服务内容描述')->required(),
            Elm::frameImage('image', '服务条款图标(100*100px)：', '/' . config('admin.admin_prefix') . '/setting/uploadPicture?field=image&type=1')->value($formData['image']??'')->icon('el-icon-camera')->modal(['modal' => false])->width('1000px')->height('600px')->required(),
