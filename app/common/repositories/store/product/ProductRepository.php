...
class ProductRepository extends BaseRepository
{
    ...
    public function freeTrial(int $id, array $data, int $merId, $admin_info = [])
    {
        ...
        Db::transaction(function () use ($id, $data, $settleParams, $ret, $admin_info) {
            $this->save($id, $settleParams, null, ['admin_info' => $admin_info], 0);
        ...
        try {
            ...
            foreach ($data['attrValue'] as $value) {
                ...
                if ($isUpdate && isset($value['unique'])) {
                    foreach ($productAttrValue as $attrValue) {
                        if ($attrValue['unique'] == $value['unique']) {
                            if ($new_price > $attrValue['price']) {
                                $update_infos[OperateLogRepository::MERCHANT_INC_PRODUCT_PRICE] .= $attrValue['sku'] . '价格增加了' . bcsub($new_price, $attrValue['price'], 2) . ',';
                            }
                            if ($new_price < $attrValue['price']) {
                                $update_infos[OperateLogRepository::MERCHANT_DEC_PRODUCT_PRICE] .= $attrValue['sku'] . '价格增加了' . bcsub($attrValue['price'], $new_price, 2) . ',';
                            }
                            if ($new_stock > $attrValue['stock']) {
                                $update_infos[OperateLogRepository::MERCHANT_INC_PRODUCT_STOCK] .= $attrValue['sku'] . '库存增加了' . ($new_stock - $attrValue['stock']) . ',';
                            }
                            if ($new_stock < $attrValue['stock']) {
                                $update_infos[OperateLogRepository::MERCHANT_DEC_PRODUCT_STOCK] .= $attrValue['sku'] . '库存减少了' . ($attrValue['stock'] - $new_stock) . ',';
                            }
                        }
                    }
            ...
            throw new ValidateException('规格错误 ：' . $exception->getMessage() . $exception->getFile() . $exception->getLine());
        }
        return $result;
    }
    ...
    public function getAdminOneProduct(int $id, ?int $activeId, $conType = 0)
    {
        $with = ['attr', 'attrValue.productCdkey', 'oldAttrValue', 'merCateId.category', 'storeCategory', 'brand', 'temp', 'seckillActive',
            ...
            'guarantee.templateValue.value', 'getFormName'
        ];
...
//        1 出售中 2 仓库中 3 已售罄 4 警戒库存 5 回收站 6 待审核 7 审核未通过
    ...
    public function getSeckillList(?int $merId, array $where, int $page, int $limit)
    {
        ...
        $list = $query->page($page, $limit)->setOption('field', [])->field($this->filed)->order('sort DESC')->select()
            ...
            })->append(['seckill_status', 'us_status']);
        return compact('count', 'list');
    }
    ...
    public function getAdminSeckillList(?int $merId, array $where, int $page, int $limit)
    {
        ...
        $list = $query->page($page, $limit)
            ->field('Product.*,U.star,U.rank,U.sys_labels')->select()
            ->each(function ($item) {
                if (!$item->sys_labels) {
                    $item->sys_labels = [];
                } else {
    ...
    public function getCateIdByCategory(array $where)
    {
        ...
        unset($where['mer_id']);
        $query = $this->dao->search($mer_id, $where);
        return $query->group('cate_id')->column('cate_id');
    }
    ...
    public function getProductShow($productId, $product, $activityId, $uid)
    {
        ...
        if (!isset($res) || !$res) {
            ...
            if (in_array($product['product_type'], [0, 2, 4])) {
                ...
                $active = $storeActivityRepository->getActivityBySpu(StoreActivityRepository::ACTIVITY_TYPE_ATMOSPHERE, $res['spu_id'], $product['cate_id'], $product['mer_id'], $spu['sys_labels']);
...
//        if ($data['cart_num'] !== 1) throw new ValidateException('秒杀商品只能购买一个');
    ...
    public function isFormatAttr(array $data, int $productId, $skuValue = [])
    {
        if ($productId) {
            $make = app()->make(ProductAttrValueRepository::class);
