...
class ProductReplyRepository extends BaseRepository
{
    ...
    public function form(?int $productId)
    {
        ...
        $rule[] = Elm::input('comment', '评价文字：')->type('textarea')->placeholder('请输入评价文字');
        $rule[] = Elm::rate('product_score', '商品分数：', 5)->max(5);
        $rule[] = Elm::rate('service_score', '服务分数：', 5)->max(5);
        $rule[] = Elm::rate('postage_score', '物流分数：', 5)->max(5);
        $rule[] = Elm::frameImage('avatar', '用户头像：', '/' . config('admin.admin_prefix') . '/setting/uploadPicture?field=avatar&type=1')->width('1000px')->height('600px')->props(['footer' => false])->icon('el-icon-camera')->modal(['modal' => false]);
